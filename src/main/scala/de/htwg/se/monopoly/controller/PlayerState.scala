...
object FreePlayerState extends PlayerState {
  ...
  override def rollDice(dice: Dice): PlayerState= {
      Game.board.movePlayer(dice.getFaceValue, Game.controller.gameState.currentPlayer)
      FreePlayerState
  }

  override def stringRollDice(dice: Dice): String = {
    if (Game.controller.getCurrentDice.hasDoublets) {
      if (dice.getFaceValue + Game.board.players(Game.controller.gameState.currentPlayer).getLocation == Game.board.getTotalNumberOfSpaces) {
...
object JailedPlayerState extends PlayerState {
  ...
  override def rollDice(dice: Dice): PlayerState = {
    if (dice.hasDoublets) {
      Game.board.setPlayerJailedOrUnJailed(Game.controller.gameState.currentPlayer, jailed = false)
  ...
  override def stringRollDice(dice: Dice): String = {
    if (dice.hasDoublets) {
      val freePlayerState: PlayerState = determinePlayerState(Game.board.players(Game.controller.gameState.currentPlayer))
